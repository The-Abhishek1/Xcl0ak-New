...
export function Topbar() {
  ...
  }, [loggedIn])  // re-runs when login state changes
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 h-[52px] z-[100] flex items-center px-4 gap-3"
      ...
      <div className="flex items-center gap-2 shrink-0">
        ...
        {/* Sync */}
        <button onClick={doSync} title="Sync threat feeds"
          ...
          style={{
            ...
            color:      sync==='done' ? '#00ffaa' : '#64748b', fontSize:'14px',
          }}>
          {sync==='syncing' ? '⟳' : sync==='done' ? '✓' : '↻'}
        </button>

        {/* Notifications — only show bell+badge when logged in */}
        <div ref={notifRef} className="relative">
          <button onClick={()=>setNotifOpen(v=>!v)} title="Notifications"
            ...
            {/* Only show unread badge when logged in AND there are unread notifications */}
          ...
          {notifOpen && (
            <div className="absolute right-0 top-11 w-72 rounded-xl shadow-2xl z-50"
              ...
              {!loggedIn ? (
                <div className="px-4 py-6 text-center">
                  <p className="font-mono text-[10px] text-slate-600 mb-3">Sign in to see notifications</p>
                  <Link href="/login" onClick={()=>setNotifOpen(false)}
                    className="font-mono text-[10px] text-accent hover:underline">Sign in →</Link>
                </div>
        ...
        <div ref={userRef} className="relative">
          {loggedIn && user ? (
            <>
              ...
              {userOpen && (
                <div className="absolute right-0 top-11 w-52 rounded-xl shadow-2xl z-50"
                  style={{background:'rgba(6,9,16,0.98)',border:'1px solid rgba(255,255,255,0.1)',backdropFilter:'blur(24px)'}}>
