...
const DIFF_COLOR: Record<string,string> = {
  easy:'#00ffaa', medium:'#ffd700', hard:'#ff3a5c',
}
...
export default function CTFPage() {
  ...
  async function submitFlag() {
    ...
    } catch { setFlagMsg({ ok:false, text:'Submission failed.' }) }
  ...
  return (
    <div className="p-3 sm:p-5">
      ...
      <div className="flex items-start justify-between mb-4 gap-3 flex-wrap">
        <div>
          ...
          <p className="font-mono text-[11px] text-slate-500 mt-1">
            {challenges.length} active challenges · Submit flags to earn XP
          </p>
        </div>
        {/* XP stats + Submit button */}
        <div className="flex items-center gap-2 flex-wrap">
          <div className="glass-sm px-3 py-2 text-center">
            <div className="font-mono text-lg font-bold text-yellow-400">{totalXP}</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-4">
        ...
        <div className="space-y-2.5">
          ...
          {!loading && visible.length===0 && (
            <div className="glass p-10 text-center">
              <div className="font-mono text-[13px] text-slate-500 mb-3">No challenges yet.</div>
              <Link href="/ctf/submit" className="font-mono text-[11px] text-accent hover:underline">
            ...
            <div key={ch.id}
              ...
              <div className="flex items-start gap-3">
                <div className="text-2xl mt-0.5 shrink-0">
                  {ch.category==='web'?'🌐':ch.category==='pwn'?'💣':ch.category==='crypto'?'🔐':ch.category==='forensics'?'🔍':ch.category==='reverse'?'⚙':ch.category==='osint'?'🕵':'🧩'}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                    ...
                    {ch.expiresAt && (
                      <span className="font-mono text-[10px] text-orange-400">⏱ {timeAgo(ch.expiresAt)}</span>
                    )}
                    {ch.solved && (
                      <span className="font-mono text-[9px] px-1.5 py-[1px] rounded border bg-accent/10 text-accent border-accent/25 ml-auto">✓ SOLVED</span>
                    )}
                  </div>
                ...
                <div className="text-right shrink-0">
                  ...
                  {ch._count && <div className="font-mono text-[9px] text-slate-600 mt-1">{ch._count.solves} solves</div>}
                </div>
              </div>
            </div>
          ))}
        ...
        {/* Sidebar: flag submit + scoreboard */}
        <div className="space-y-4">
            ...
            <div className="glass p-4">
              ...
              {flagMsg && (
                <div className={`mt-2 p-2.5 rounded-lg font-mono text-[11px] border ${
                  flagMsg.ok?'bg-accent/8 border-accent/25 text-accent':'bg-red-500/8 border-red-500/25 text-red-400'
                }`}>{flagMsg.text}</div>
              )}
            </div>
          )}
          ...
          <div className="glass overflow-hidden">
            ...
            <div className="py-1">
              {leaderboard.length===0 ? (
                <div className="px-4 py-6 text-center font-mono text-[10px] text-slate-600">No researchers yet</div>
                ...
                return (
                  <div key={user.id} className="flex items-center gap-2.5 px-4 py-2 border-b border-white/[0.03]">
                    <span className="font-mono text-[10px] font-bold w-5" style={{color:c}}>#{user.rank??i+1}</span>
                    ...
                    <span className="font-mono text-[10px] font-bold text-slate-500">{user.reputation?.toLocaleString()} XP</span>
                  </div>
                )
              })}
            </div>
          ...
          <div className="glass p-4 text-center">
            ...
            <div className="font-mono text-[12px] font-bold text-slate-300 mb-1">Create a Challenge</div>
            <div className="font-mono text-[10px] text-slate-600 mb-3">Share your knowledge with the community</div>
            <Link href="/ctf/submit"
