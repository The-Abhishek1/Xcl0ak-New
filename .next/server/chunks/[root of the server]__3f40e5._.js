 ...
 */ __turbopack_esm__({
    ...
    "sendCTFApprovedEmail": (()=>sendCTFApprovedEmail),
    "sendCTFSolvedEmail": (()=>sendCTFSolvedEmail),
    "sendExploitApprovedEmail": (()=>sendExploitApprovedEmail),
    "sendExploitSubmittedEmail": (()=>sendExploitSubmittedEmail),
    "sendPasswordResetEmail": (()=>sendPasswordResetEmail),
    "sendPaymentEmail": (()=>sendPaymentEmail),
    "sendScanCompleteEmail": (()=>sendScanCompleteEmail),
...
function badge(text, color = '#00ffaa') {
...
function btn(text, url, color = '#00ffaa') {
...
function stat(label, value, color = '#00ffaa') {
...
async function sendCTFSolvedEmail(to, username, challengeTitle, points, totalRep) {
...
async function sendExploitSubmittedEmail(to, username, exploitTitle) {
...
async function sendExploitApprovedEmail(to, username, exploitTitle, exploitId) {
...
async function sendCTFApprovedEmail(to, username, challengeTitle) {
...
async function sendScanCompleteEmail(to, username, target, scanId, riskLevel, findingsCount) {
...
async function sendPaymentEmail(to, username, tier, amount, paymentId) {
...
async function sendWeeklyReport(to, username, stats) {
    ...
    ${stats.topCVEs.slice(0, 3).map((cve)=>`<div style="padding:8px 12px;margin:4px 0;background:rgba(255,58,92,0.05);border:1px solid rgba(255,58,92,0.15);border-radius:6px;font-size:12px;color:#ff8c42;">${cve}</div>`).join('')}` : ''}
    ${divider()}
    ${btn('View Dashboard', 'https://xcloak.tech/dashboard')}
    ${btn('Run a Scan', 'https://xcloak.tech/scan/new', '#00aaff')}
  `);
    await send(to, `📊 XCloak Weekly Report — w/e ${new Date().toLocaleDateString('en-IN', {
        day: 'numeric',
        month: 'short'
    })}`, html);
}
...
"[project]/src/app/api/v1/auth/forgot-password/route.ts [app-route] (ecmascript)": ((__turbopack_context__) => {
"use strict";
...
async function POST(req) {
    ...
    if (user) {
        ...
        await __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$prisma$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["prisma"].user.update({
            ...
            data: {
                resetToken: (0, __TURBOPACK__imported__module__$5b$externals$5d2f$crypto__$5b$external$5d$__$28$crypto$2c$__cjs$29$__["createHash"])('sha256').update(token).digest('hex'),
                resetExpires: expires
            }
        }).catch(()=>null) // field may not exist yet — see schema note below
        ;
        const resetUrl = `${("TURBOPACK compile-time value", "http://localhost:3000")}/reset-password?token=${token}&email=${encodeURIComponent(email)}`;
        await (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$email$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["sendPasswordResetEmail"])(email, user.alias, resetUrl);
    }
...
//# sourceMappingURL=%5Broot%20of%20the%20server%5D__3f40e5._.js.map