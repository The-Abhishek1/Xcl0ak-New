...
export function Sidebar() {
  ...
  return (
    <aside className="fixed left-0 top-[52px] bottom-0 w-[220px] z-[90] flex flex-col py-4 overflow-y-auto hidden md:flex"
      style={{background:'rgba(3,5,10,0.65)',backdropFilter:'blur(20px)',borderRight:'1px solid rgba(255,255,255,0.06)'}}>
      {SECTIONS.map(sec => (
        <div key={sec.label} className="mb-5 px-3">
          <div className="font-mono text-[9px] tracking-[0.15em] text-slate-600 uppercase px-2 mb-1.5">{sec.label}</div>
          {sec.items.map(item => {
            ...
            return (
              <Link key={item.href} href={item.href}
                ...
                style={active
                  ? {color:'#00ffaa',borderLeft:'2px solid #00ffaa',paddingLeft:'8px',background:'linear-gradient(90deg,rgba(0,255,170,0.08),transparent)'}
                  : {color:'#64748b'}}>
                <span className="w-[18px] text-center text-sm shrink-0">{item.icon}</span>
                <span className="flex-1">{item.label}</span>
                {item.badge && (
                  <span className={`font-mono text-[9px] px-1.5 py-[1px] rounded-[3px] ${item.badge==='LIVE'?'bg-red-500/20 text-red-400':'bg-green-500/10 text-green-400'}`}>
                    {item.badge}
                  </span>
                )}
              </Link>
      ...
      <div className="mt-auto mx-3 p-3 rounded-lg" style={{background:'rgba(0,255,170,0.04)',border:'1px solid rgba(0,255,170,0.08)'}}>
