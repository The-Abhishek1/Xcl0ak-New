...
function PricingPage() {
    ...
    const handleUpgrade = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useCallback"])({
        "PricingPage.useCallback[handleUpgrade]": async (tier)=>{
            ...
            try {
                ...
                await new Promise({
                    "PricingPage.useCallback[handleUpgrade]": (resolve, reject)=>{
                        const rzp = new window.Razorpay({
                            ...
                            handler: {
                                "PricingPage.useCallback[handleUpgrade]": async (response)=>{
                                    try {
                                        ...
                                        const fresh = await fetch('/api/eso/auth/me', {
                                            ...
                                            "PricingPage.useCallback[handleUpgrade]": (r)=>r.json()
                                        }["PricingPage.useCallback[handleUpgrade]"]).catch({
                                            "PricingPage.useCallback[handleUpgrade]": ()=>null
                                        }["PricingPage.useCallback[handleUpgrade]"]);
                                        if (fresh && token) {
                            ...
                            modal: {
                                ondismiss: {
                                    "PricingPage.useCallback[handleUpgrade]": ()=>reject(new Error('Payment cancelled'))
                                }["PricingPage.useCallback[handleUpgrade]"]
                            }
                        });
                        rzp.open();
    ...
    }["PricingPage.useEffect"], []) // eslint-disable-line
    ;
...
_s(PricingPage, "Hu+HDj2MN2SaA8lTmj2PAgJjPA0=", false, function() {
    return [
