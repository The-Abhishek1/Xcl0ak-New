"use strict";exports.id=4702,exports.ids=[4702],exports.modules={14702:(e,o,t)=>{t.d(o,{sendEmail:()=>s,templates:()=>n});let a=process.env.EMAIL_FROM??"XCloak <admin@xcloak.tech>",r=process.env.RESEND_API_KEY??"";async function s(e){if(!r)return console.log("[email] No RESEND_API_KEY — would have sent:",e.subject,"→",e.to),!1;try{let o=await fetch("https://api.resend.com/emails",{method:"POST",headers:{Authorization:`Bearer ${r}`,"Content-Type":"application/json"},body:JSON.stringify({from:a,to:Array.isArray(e.to)?e.to:[e.to],subject:e.subject,html:e.html,reply_to:e.replyTo})});if(!o.ok){let e=await o.text();return console.error("[email] Resend error:",e),!1}return!0}catch(e){return console.error("[email] fetch failed:",e),!1}}let l=e=>`
<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
...
`,n={welcome:e=>({subject:"\uD83D\uDEE1 Welcome to XCloak — your security journey starts now",html:l(`
      <h2>Welcome, ${e}!</h2>
    ...
    `)}),exploitApproved:(e,o)=>({subject:`✅ Your exploit "${o}" is now live`,html:l(`
      <h2>Exploit Approved!</h2>
      <p>Your submission <strong style="color:#00ffaa">${o}</strong> has been reviewed and approved by our team.</p>
    ...
    `)}),exploitRejected:(e,o,t)=>({subject:`❌ Exploit submission update: "${o}"`,html:l(`
      <h2>Submission Not Approved</h2>
      <p>Unfortunately, your exploit <strong style="color:#ff3a5c">${o}</strong> was not approved.</p>
      ${t?`<p><strong style="color:#e2e8f0">Reason:</strong> ${t}</p>`:""}
    ...
    `)}),ctfSolved:(e,o,t)=>({subject:`🏆 CTF Solved: ${o} (+${t} pts)`,html:l(`
      <h2>Challenge Complete!</h2>
      <p>You solved <strong style="color:#facc15">${o}</strong> and earned
         <span class="tag" style="background:rgba(250,204,21,0.1);border:1px solid rgba(250,204,21,0.3);color:#facc15;">+${t} points</span></p>
      <p>Keep going — more challenges await on the leaderboard.</p>
      <a href="https://xcloak.tech/ctf" class="btn">Next challenge →</a>
    `)}),learningPathApproved:(e,o)=>({subject:`🎉 Your learning path "${o}" is live!`,html:l(`
      <h2>Path Approved!</h2>
      <p>Your learning path <strong style="color:#00ffaa">${o}</strong> has been approved and is now live for the community.</p>
    ...
    `)}),learningPathRejected:(e,o,t)=>({subject:`Learning path update: "${o}"`,html:l(`
      <h2>Path Not Approved</h2>
      <p>Your learning path <strong style="color:#ff3a5c">${o}</strong> needs some changes before it can go live.</p>
      ${t?`<p><strong style="color:#e2e8f0">Feedback:</strong> ${t}</p>`:""}
      <a href="https://xcloak.tech/learn" class="btn">Edit and resubmit →</a>
    `)}),ctfApproved:(e,o)=>({subject:`✅ CTF Challenge approved: "${o}"`,html:l(`
      <h2>CTF Challenge Live!</h2>
      <p>Your challenge <strong style="color:#00ffaa">${o}</strong> has been approved and is now live.</p>
      <p>The community can now attempt to solve it and earn points!</p>
      <a href="https://xcloak.tech/ctf" class="btn">View challenge →</a>
    `)}),ctfRejected:(e,o,t)=>({subject:`CTF Challenge update: "${o}"`,html:l(`
      <h2>Challenge Not Approved</h2>
      <p>Your CTF challenge <strong style="color:#ff3a5c">${o}</strong> was not approved.</p>
      ${t?`<p><strong style="color:#e2e8f0">Reason:</strong> ${t}</p>`:""}
    ...
    `)}),scanComplete:(e,o,t)=>({subject:`🔍 Scan complete: ${o} — ${t} finding${1!==t?"s":""}`,html:l(`
      <h2>Scan Complete</h2>
      <p>Your scan of <strong style="color:#00aaff">${o}</strong> has finished.</p>
      <p>We found <span class="tag" style="background:rgba(255,58,92,0.1);border:1px solid rgba(255,58,92,0.25);color:#ff3a5c;">${t} finding${1!==t?"s":""}</span>. Review them in your dashboard.</p>
      <a href="https://xcloak.tech/findings" class="btn">View findings →</a>
    `)}),passwordReset:(e,o)=>({subject:"\uD83D\uDD11 Reset your XCloak password",html:l(`
      <h2>Password Reset</h2>
      <p>We received a request to reset the password for <strong style="color:#e2e8f0">${e}</strong>.</p>
      <p>This link expires in 1 hour. If you didn't request this, ignore this email.</p>
      <a href="${o}" class="btn">Reset password →</a>
    `)})}}};